...
- **Online learning / model refresh**: fraud patterns shift weekly. Static models decay. PayPal retrains on a rolling window and monitors score distributions in real-time for drift.

**What failed first:** The first gradient boosting model had excellent AUC (0.97) but its calibration was poor — probabilities were not reliable. A threshold of 0.5 gave mediocre precision/recall. Once they calibrated (Platt scaling, isotonic regression) and set the threshold from the cost ratio, operational performance improved significantly. See [doc 04](../04-evaluation-and-validation.md).
...
**Approach:** **Merchant categorisation via NLP**: merchant descriptor strings (the raw string from card networks like "COSTA COFFEE LONDON 1234") are messy, truncated, and inconsistent. Monzo trained a multi-class text classifier (character-level CNN, later fine-tuned BERT) on a human-labelled corpus of merchant strings → category.
...
- **Personalised anomaly detection**: "you spent £200 more than usual this month" requires a per-user baseline, not a population baseline. This is a personalized z-score over recent monthly spend. See [doc 13](../13-anomaly-detection.md).
