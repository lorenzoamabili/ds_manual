...
- **Personalisation at scale**: Netflix runs 250+ A/B tests simultaneously; the recommendation system (a hybrid of matrix factorisation + bandit exploration) is evaluated not on RMSE but on whether viewing a recommendation *leads to completion*, which is a stronger retention signal.
...
**Approach:** Spotify uses a two-stage architecture: (1) **candidate generation** via collaborative filtering on implicit feedback (play, skip, save, share — not just listens); (2) **ranking** with a gradient-boosted model incorporating audio features (CNN on raw audio spectrograms), NLP on playlist co-occurrence ("songs that appear in playlists called 'chill morning' cluster differently from 'workout'"), and user context (time of day, device, recent history).
...
**Key technical decisions:**
- **Feature engineering**: last-active hour, day-of-week pattern, streak length, language being learned, time-zone. Streak length is a *moderating variable* — users with streaks > 30 days respond less to reminders (intrinsically motivated) so the bandit learns to suppress for them.
